...
export default function FratList() {
  ...
  return (
    <div className="bg-[#E6CFDC] flex flex-col items-center min-h-screen p-6 font-['Playfair_Display']">
      <h1 className="text-3xl font-bold text-[#5A2A51] mb-8">Fraternity List</h1>
            ...
            >
              {frat.image && (
                <img
                  src={frat.image}
                  alt={frat.name}
                  className="w-40 h-40 object-cover rounded-full mb-4 border-4 border-[#5A2A51]"
                />
              )}
              <h3 className="text-xl font-bold text-[#5A2A51] text-center">
                {frat.name}
              </h3>
            </Link>
          ))
